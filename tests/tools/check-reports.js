...
if (!EXTRACTED) {
  ...
  const blocks = () => ({
    HEADER: [
      ...
      `                and packaging only.`,
      `DEPLOYED        NOT BY THIS PACKAGE — no step in producing it deployed`,
      `                anything, and nothing here was run against production`,
      `                data. Every test drives the shipped code against a`,
      `                controlled API in a local browser. Whether the accepted`,
      `                application is live is a separate fact, recorded in`,
      `                docs/control/CANONICAL-STATE.json under "production".`,
    ].join('\n'),
