# QUOTATION.DNC — round 7, part 1: AI extraction
...
that the rule stands: `SS304 → N/A`, `SS316 → N/A`, no PL, no HDG, no ZP,
whatever the source document says, with the source wording kept only as
evidence — which is exactly what the code does. Part 2 of this round then
found that the rule was **not** being enforced outside Quick Add and closed
those gaps; see `final-report.md` §4.

**The model's half of both cases is unverified against the live API.** Everything
downstream of the model's answer is proven by 146 assertions against fixtures
shaped exactly like the reported failures. What still needs one live run with a
key is whether the model, with the new schema and instructions, actually
classifies the hook as `J_BOLT` and reads the rotated table's merged cells the
way the prompt now asks. The prompt text and the schema are asserted in
`tests/php/ai_extract.test.php`; the model's behaviour cannot be.
...
Branch `claude/quotation-dnc-audit-repair-ashi82`. **Superseded within the same
round:** the Quick Add repair sits on top of it, and the commit to deploy is the
one in `final-report.md` §10.
