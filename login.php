...
<div class="wrap">
  ...
  <div class="card">
    <h1>Sign In <span class="zh">/ 登录</span></h1>
    <p class="sub">Enter the shared staff account to continue.</p>

    <?php if ($error !== ''): ?>
      <div class="msg err" role="alert"><span>⚠️</span><span><?= htmlspecialchars($error, ENT_QUOTES, 'UTF-8') ?></span></div>
    <?php elseif ($justLoggedOut): ?>
      <div class="msg ok" role="status"><span>✓</span><span>You have been signed out on this device. / 已在此设备登出。</span></div>
    <?php endif; ?>

    <form method="post" action="login.php" autocomplete="on">
      <input type="hidden" name="next" value="<?= htmlspecialchars($next, ENT_QUOTES, 'UTF-8') ?>">
      <div class="field">
        <label for="username">Username <span class="zh">/ 用户名</span></label>
        <input type="text" id="username" name="username" autocomplete="username"
               autocapitalize="none" autocorrect="off" spellcheck="false" required autofocus>
      </div>
      <div class="field">
        <label for="password">Password <span class="zh">/ 密码</span></label>
        <input type="password" id="password" name="password" autocomplete="current-password" required>
      </div>
      <div class="row">
        <input type="checkbox" id="showpw" onclick="document.getElementById('password').type=this.checked?'text':'password'">
        <label for="showpw">Show password / 显示密码</label>
      </div>
      <button type="submit">Sign In / 登录</button>
    </form>

    <p class="foot">Stays signed in for about 7 days on this device.<br>此设备约保持登录 7 天。</p>
  </div>
</div>
</body>
</html>
